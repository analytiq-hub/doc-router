...
**Microsoft SharePoint OAuth** (`microsoftSharePointOAuth2Api`, experimental) extends `microsoftOAuth2Api` with n8n-style resource scope `openid offline_access https://{{subdomain}}.sharepoint.com/.default`. **Subdomain** is required before Connect (tenant slug from `https://tenant123.sharepoint.com`). The Entra app must include **SharePoint** delegated permissions (e.g. `AllSites.Read` / `AllSites.Write` under Office 365 SharePoint Online — not Graph `Sites.Read.All`). `Sites.Search.All` is search-only; `Sites.Selected` requires per-site grants. Admin consent where required, then reconnect. SharePoint flow nodes call SharePoint REST v2.0 at `https://{subdomain}.sharepoint.com/_api/v2.0/` (and site-scoped paths); which site collection to use is set per node (**Site** parameter, e.g. `root`, `/sites/Team`, or `contoso.sharepoint.com:/sites/Team`). Reconnect after subdomain, permissions, or scope changes. Test request: `GET https://{{ credentials.subdomain }}.sharepoint.com/_api/v2.0/sites/root`.
...
4. **Frontend opens a popup** (`window.open`) to the provider's authorization page (initiate body `{ "popup": true }` by default).
...
7. **Popup mode:** API redirects the popup to `{NEXTAUTH_URL}/oauth/flow-callback?status=success|error` (Next.js **Route Handler** — static HTML, no app shell). That page posts on `BroadcastChannel('flow-oauth-callback')` and closes. The credential modal refreshes the credential row. **Non-popup** (legacy): redirect to `{NEXTAUTH_URL}/orgs/{orgId}/flows?tab=credentials&flow_oauth=…` (full-page).
