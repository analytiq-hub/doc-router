...
def _create_prompt(name: str) -> None:
    r = client.post(
        f"/v0/orgs/{TEST_ORG_ID}/prompts",
        json={"name": name, "content": "test content", "model": "gpt-4o-mini", "tag_ids": []},
        headers=get_auth_headers(),
    )
...
@pytest.mark.asyncio
async def test_prompt_name_search_match_rank_exact_prefix_substring(test_db, mock_auth, setup_test_models):
    """
    Rank 0 (exact) beats rank 1 (prefix) beats rank 2 (substring).
    Prompts are created in ascending-_id order so without ranking the default
    ...
    """
    _create_prompt("invoice")
    _create_prompt("invoice processor")
    _create_prompt("my invoice")

    r = _list_prompts({"name_search": "invoice", "limit": 50})
    names = [p["name"] for p in r["prompts"]]
...
@pytest.mark.asyncio
async def test_prompt_name_search_case_insensitive_exact(test_db, mock_auth, setup_test_models):
    """
