...
@pytest.mark.asyncio
async def test_put_flow_rename_only_change_does_not_create_revision(test_db, mock_auth):
    """Renaming a node must not bump flow_version; the new label is patched on the latest revision."""
    ...
    nodes = [_std_node("t1", "Original label", "flows.trigger.manual", 0)]
    conns: dict = {}
    r1 = client.put(
        f"/v0/orgs/{TEST_ORG_ID}/flows/{flow_id}",
        json={
    ...
    renamed_nodes = [_std_node("t1", "Renamed label", "flows.trigger.manual", 0)]
    db = ad.common.get_async_db()
    count_before = await db.flow_revisions.count_documents({"flow_id": flow_id})
    ...
    assert stored is not None
    assert stored["flow_version"] == version_before
    assert stored["nodes"][0]["name"] == "Renamed label"
