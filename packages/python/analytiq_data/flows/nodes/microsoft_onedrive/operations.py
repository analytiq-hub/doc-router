"""Microsoft Graph OneDrive operations for ``flows.microsoft_onedrive``."""
...
async def execute_microsoft_onedrive_item(
    context: "ad.flows.ExecutionContext",
    node: dict[str, Any],
    params: dict[str, Any],
...
async def _run_file(
    ...
    *,
    ...
    if operation == "share":
        ...
        return await _graph(
            context, token, "POST", f"/drive/items/{file_id}/createLink", body=body
        )

    if operation == "upload":
        return await _file_upload(context, token, params, item, item_index=item_index)

    if operation == "rename":
        return await _rename_item(context, token, params)
...
async def _run_folder(
    ...
    if operation == "create":
        ...
        for name in names:
            ...
            last = await _graph(context, token, "POST", endpoint, body=body)
            if not last.get("id"):
                break
    ...
    if operation == "share":
        ...
        return await _graph(
            context, token, "POST", f"/drive/items/{folder_id}/createLink", body=body
        )
...
async def _read_input_binary(
    ...
    *,
    ...
    if ref is None:
        raise RuntimeError(
            f"Microsoft OneDrive expected item.binary[{prop!r}] but it was missing "
    ...
    if ref.data is not None:
        ...
        raise RuntimeError("Microsoft OneDrive binary upload requires item binary data.")
