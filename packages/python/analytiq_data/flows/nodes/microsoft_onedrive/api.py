"""Microsoft Graph OneDrive HTTP client for ``flows.microsoft_onedrive``."""
...
def onedrive_graph_user_hint(graph_message: str | None) -> str | None:
    """Actionable hint for common Graph OneDrive failures."""
...
def format_onedrive_graph_error(exc: MicrosoftGraphApiError) -> str:
    hint = onedrive_graph_user_hint(exc.graph_message)
    if hint:
        return hint
    return str(exc)
...
async def resolve_oauth_access_token_for_org(
    ...
    cred_id = bindings.get("microsoftOneDriveOAuth2Api")
    if not cred_id:
        raise RuntimeError(
            "Microsoft OneDrive requires a microsoftOneDriveOAuth2Api credential on the node."
        )
    ...
    if not token:
        raise RuntimeError(
            "Microsoft OneDrive OAuth2 credential has no access token. "
...
async def microsoft_graph_request(
    ...
    *,
    ...
    req_url = url if url else f"{_GRAPH_ME}{path}"
...
async def microsoft_graph_request_with_response(
    token: str,
    method: str,
    path: str,
    *,
...
async def microsoft_graph_request_all_items(
...
async def microsoft_graph_request_all_items_delta(
    context: ExecutionContext | None,
    token: str,
    link: str,
    since_iso: str,
    ...
    *,
    ...
    """Delta query with created/updated filtering (n8n ``microsoftApiRequestAllItemsDelta``)."""
...
async def get_onedrive_folder_path(
    context: ExecutionContext | None,
    token: str,
    folder_id: str,
    *,
    ...
    """Folder path for nested watch filtering (n8n ``getPath``)."""

    data = await microsoft_graph_request(
        token,
        "GET",
