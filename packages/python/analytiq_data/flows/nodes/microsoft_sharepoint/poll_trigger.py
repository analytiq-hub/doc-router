"""Microsoft SharePoint trigger poll logic (site document library delta)."""
...
from .api import (
    ...
    resolve_sharepoint_subdomain_for_org,
)
from .helpers import sharepoint_site_id
...
async def _probe_site_drive_delta_sync(
    token: str,
    site_base: str,
    *,
    ...
    page = await graph_request(
        ...
        "GET",
        "",
        url=site_drive_delta_latest(site_base),
...
async def poll_microsoft_sharepoint_trigger(
    ...
    subdomain = await resolve_sharepoint_subdomain_for_org(context.organization_id, node)
    site_base = sharepoint_rest_api_base(subdomain, sharepoint_site_id(params))
    ...
    last_link = str(context.get_static(LAST_LINK_KEY) or site_drive_delta_latest(site_base))
    ...
    try:
        if testing:
            await _probe_site_drive_delta_sync(
                token, site_base, trace_node_id=trace_id
            )
            ...
            if watch == "selectedFile" and file_id:
                ...
                await get_drive_folder_path(
                    None, token, folder_id, mailbox_base=site_base, trace_node_id=trace_id
                )
        ...
        if manual:
            page = await graph_request(
                ...
                "GET",
                "",
                url=site_drive_delta_root(site_base),
                trace_node_id=trace_id,
            )
    ...
    if (
        ...
        if watch == "oneSelectedFolder":
            response_data = [
                x for x in response_data if str(x.get("id") or "") == folder_id
            ]
        else:
            response_data = [
                x
