"""Microsoft Outlook flow node (type version 1)."""
...
from .operations import execute_microsoft_outlook_item
...
class FlowsMicrosoftOutlookNode:
    """Consume Microsoft Outlook API (``microsoftOutlookOAuth2Api``)."""

    key = "flows.microsoft_outlook"
    ...
    async def execute(
        ...
        for i, item in enumerate(slot):
            try:
                result = await execute_microsoft_outlook_item(
