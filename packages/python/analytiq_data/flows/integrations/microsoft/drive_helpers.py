"""Microsoft Graph OneDrive / drive helpers (shared across drive-based nodes)."""
...
def normalize_drive_item_id(raw: Any) -> str:
...
def encoded_drive_item_content_path(parent_id: str, file_name: str) -> str:
    return f"/drive/items/{parent_id}:/{quote(str(file_name), safe='')}:/content"


def simplify_drive_item(item: dict[str, Any]) -> dict[str, Any]:
