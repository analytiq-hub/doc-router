...
"""Editor test run for schedule triggers (one tick without activating)."""
...
async def enqueue_schedule_trigger_test_run(
    analytiq_client,
    *,
    organization_id: str,
    flow_id: str,
    ...
    """
    Fire ``on_schedule_tick`` once for the given trigger node and enqueue a test run.
    ...
    if node.get("type") != "flows.trigger.schedule":
        raise ad.flows.FlowValidationError(f"Node {trigger_node_id!r} is not a schedule trigger")
    ...
    items = await nt.on_schedule_tick(ctx, node)
    if ctx.data_changed:
